...
public class MemberController extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		gson = new CustomTimeGson().createGson();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch (action) {
        	case "/searchByStudentId":
        		...
        		break;
        	case "/searchStudentByKeyword":
        		searchStudentByKeyword(request, response);
        		break;
        	case "/listMember":
	...
	private void searchStudentByKeyword(HttpServletRequest request, HttpServletResponse response) throws IOException {
		String keyword = request.getParameter("keyword");
	    
	    if (keyword!=null) {
	        JsonResponse<List<Student>> jsonResponse = null;
			try {
				...
				for (Student student : studentList) {
					...
					}else if (student.getStudentCode().toLowerCase().contains(keyword.toLowerCase())){
						searchList.add(student);
					}
				}
	            if (studentList == null) {
	                jsonResponse = new JsonResponse<List<Student>>(false, HttpServletResponse.SC_NOT_FOUND, "Không tìm thấy sinh viên", null);            

	            } else {
	                jsonResponse = new JsonResponse<List<Student>>(true, HttpServletResponse.SC_OK, "Thành công", searchList);            
	            }
	        } catch (Exception e) {
	        	jsonResponse = new JsonResponse<>(false, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Có lỗi xảy ra", null);
	        	e.printStackTrace();
	...
	private void addMember(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			if (isExist(projMem)) {
				jsonResponse = new JsonResponse<>(false, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Sinh viên đã được thêm trước đó", null);
			} else {
